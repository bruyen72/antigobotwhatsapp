...
import P from 'pino';
import { createDatabaseAuthState, initializeDatabase } from './database-auth-state.js';
...
// Auth state persistente para cada sessão usando banco de dados
async function createSessionAuthState(sessionId) {
  try {
    // Usar banco de dados para auth state
    const authState = createDatabaseAuthState(sessionId);
